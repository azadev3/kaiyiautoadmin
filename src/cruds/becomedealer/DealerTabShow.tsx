...
export type DataTypeDealerShow = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const DealerTabShow: React.FC = () => {
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeDealerShow) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-dealertab/${id}`, {
        status: newStatus,
      });
