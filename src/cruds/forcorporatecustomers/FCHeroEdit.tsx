...
import { DataTypeCorporateCustomers } from "./FCHeroShow";

type Props = {
  data: DataTypeCorporateCustomers;
};

const FCHeroEdit: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<DataTypeCorporateCustomers[]>({
    queryKey: ["fetchDataKeyCorporateCustomers"],
    queryFn: async () => {
      const response = await axios.get(`${endpoint}/for-corporate-customers`, {
        headers: {
  ...
  const rows = hasData
    ? fetchData.map((data: DataTypeCorporateCustomers) => ({
        id: data._id,
  ...
  const EditModal: React.FC<Props> = (props) => {
    ...
    const handleEdit = async (e: React.FormEvent, id: string) => {
      ...
      try {
        const response = await axios.put(`${endpoint}/for-corporate-customers/${id}`, formData, {
          headers: {
      ...
      <div className="edit-modal">
        ...
        <div className="edit-container">
          <form acceptCharset="UTF-8" onSubmit={(e: React.FormEvent) => handleEdit(e, props?.data?._id)}>
            <InputField
              value={titleAz}
              label="Korporativ Müştərilər Üçün səhifəsi Hero Başlıq (AZ)"
              placeholder="Məsələn: Example data..."
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
            />
            <InputField
              label="Korporativ Müştərilər Üçün səhifəsi Hero Başlıq (EN)"
              placeholder="Məsələn: Example data..."
              value={titleEn}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
            />
            <InputField
              label="Korporativ Müştərilər Üçün səhifəsi Hero Başlıq (RU)"
              placeholder="Məsələn: Example data..."
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleRu(e.target.value)}
              value={titleRu}
            />
            <InputField
              label="Korporativ Müştərilər Üçün səhifəsi Hero Açıqlama (AZ) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
            ...
            />
            <InputField
              label="Korporativ Müştərilər Üçün səhifəsi Hero Açıqlama (EN) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
            ...
            />
            <InputField
              label="Korporativ Müştərilər Üçün səhifəsi Hero Açıqlama (RU) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
