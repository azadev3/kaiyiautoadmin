...
const HeroCreate: React.FC = () => {
  ...
  const { refetch } = useQuery<DataTypeHero[]>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${endpoint}/hero`, formData, {
        headers: {
  ...
  return (
    <form acceptCharset="UTF-8" onSubmit={handleSubmit}>
      ...
      <InputImageField onChange={handleChangeMiniImage} name="miniImg" labelTitle="Mini şəkil yüklə" />
      {previewMiniImg && (
        <img
          src={previewMiniImg}
