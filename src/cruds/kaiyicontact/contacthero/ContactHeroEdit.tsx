...
import { DataTypeContactHeroShow } from "./ContactHeroShow";

type Props = {
  data: DataTypeContactHeroShow;
};

const ContactHeroEdit: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<DataTypeContactHeroShow[]>({
    queryKey: ["fetchDataKeyContactHero"],
    queryFn: async () => {
      const response = await axios.get(`${endpoint}/contacthero`, {
        headers: {
  ...
  const rows = hasData
    ? fetchData.map((data: DataTypeContactHeroShow) => ({
        id: data._id,
  ...
  const EditModal: React.FC<Props> = (props) => {
    ...
    const handleEdit = async (e: React.FormEvent, id: string) => {
      ...
      try {
        const response = await axios.put(`${endpoint}/contacthero/${id}`, formData, {
          headers: {
      ...
      <div className="edit-modal">
        ...
        <div className="edit-container">
          <form acceptCharset="UTF-8" onSubmit={(e: React.FormEvent) => handleEdit(e, props?.data?._id)}>
            <InputField
              value={titleAz}
              label="KAIYI Əlaqə səhifəsi Hero Başlıq (AZ)"
              placeholder="Məsələn: Example data..."
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
            />
            <InputField
              label="KAIYI Əlaqə səhifəsi Hero Başlıq (EN)"
              placeholder="Məsələn: Example data..."
              value={titleEn}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
            />
            <InputField
              label="KAIYI Əlaqə səhifəsi Hero Başlıq (RU)"
              placeholder="Məsələn: Example data..."
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleRu(e.target.value)}
              value={titleRu}
            />
            <InputField
              label="KAIYI Əlaqə səhifəsi Hero Açıqlama (AZ) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
            ...
            />
            <InputField
              label="KAIYI Əlaqə səhifəsi Hero Açıqlama (EN) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
            ...
            />
            <InputField
              label="KAIYI Əlaqə səhifəsi Hero Açıqlama (RU) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
