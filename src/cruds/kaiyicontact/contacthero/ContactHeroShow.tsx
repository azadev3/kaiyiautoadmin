...
export type DataTypeContactHeroShow = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const ContactHeroShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeContactHeroShow) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-contacthero/${id}`, {
        status: newStatus,
      });
...
export default ContactHeroShow;
