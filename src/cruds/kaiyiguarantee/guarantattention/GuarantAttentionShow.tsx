...
export type DataTypeGuarantAttention = {
  _id: string;
  description: {
    az: string;
    en: string;
...
const GuarantAttentionShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeGuarantAttention) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-guarantattention/${id}`, {
        status: newStatus,
      });
...
export default GuarantAttentionShow;
