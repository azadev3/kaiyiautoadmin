...
export type DataTypeGuarantDescription = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const GuarantDescriptionShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  } = useQuery<DataTypeGuarantDescription[]>({
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeGuarantDescription) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-guarantdescription/${id}`, {
        status: newStatus,
      });
    ...
    <div className="shower">
      ...
      <div className="table-container">
        <table>
          ...
          <tbody>
            {hasData
              ? fetchData?.map((data: DataTypeGuarantDescription) => (
                  <tr key={data?._id}>
                    <td>
...
export default GuarantDescriptionShow;
