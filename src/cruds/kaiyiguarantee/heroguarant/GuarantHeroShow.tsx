...
export type DataTypeGuarantHero = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const GuarantHeroShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeGuarantHero) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-guaranthero/${id}`, {
        status: newStatus,
      });
...
export default GuarantHeroShow;
