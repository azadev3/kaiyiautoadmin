...
import { DataTypeKaiyiBlog } from "./KaiyiBlogShow";
import moment from "moment";
import MyEditor from "../../../Tiptap";

const KaiyiBlogCreate: React.FC = () => {
  ...
  const { refetch } = useQuery<DataTypeKaiyiBlog[]>({
    queryKey: ["fetchDataKeyKaiyiHistoryBlog"],
    queryFn: async () => {
      const response = await axios.get(`${endpoint}/kaiyi-history-blog`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${endpoint}/kaiyi-history-blogs`, formData, {
        headers: {
  ...
  return (
    <form acceptCharset="UTF-8" onSubmit={handleSubmit}>
      <InputField
        value={titleAz}
        label="Bloq Başlıq (AZ)"
        placeholder="Məsələn: Example data..."
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
      />
      <InputField
        label="Bloq Başlıq (EN)"
        placeholder="Məsələn: Example data..."
        value={titleEn}
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
      />
      <InputField
        label="Bloq Başlıq (RU)"
        placeholder="Məsələn: Example data..."
...
export default KaiyiBlogCreate;
