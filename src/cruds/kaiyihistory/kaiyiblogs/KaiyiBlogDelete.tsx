...
const KaiyiBlogDelete: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  } = useQuery<DataTypeKaiyiBlog[]>({
    queryKey: ["fetchDataKeyKaiyiHistoryBlogs"],
    queryFn: async () => {
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await axios.delete(`${endpoint}/kaiyi-history-blogs/${id}`);
      if (response.data) {
...
export default KaiyiBlogDelete;
