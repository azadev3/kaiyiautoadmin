...
type Props = {
  data: DataTypeKaiyiBlog;
};

const KaiyiBlogEdit: React.FC = () => {
  const [loading, setLoading] = useRecoilState(LoadingState);

  //fetch data
  const { data: fetchData, isLoading } = useQuery<DataTypeKaiyiBlog[]>({
  ...
  const EditModal: React.FC<Props> = (props) => {
    ...
    const handleEdit = async (e: React.FormEvent, id: string) => {
      ...
      try {
        const response = await axios.put(`${endpoint}/kaiyi-history-blogs/${id}`, formData, {
          headers: {
