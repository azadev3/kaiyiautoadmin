...
const KaiyiHistoryBottomDelete: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await axios.delete(`${endpoint}/kaiyi-history-bottom/${id}`);
      if (response.data) {
  ...
  const columns: GridColDef[] = [
    ...
    { field: "year", headerName: "İl", width: 180 },
    {
      field: "actions",
...
export default KaiyiHistoryBottomDelete;
