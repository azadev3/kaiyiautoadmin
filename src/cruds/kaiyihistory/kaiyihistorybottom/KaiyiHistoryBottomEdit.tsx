...
type Props = {
  data: DataTypeKaiyiHistoryBottom;
};

const KaiyiHistoryBottomEdit: React.FC = () => {
  ...
  const columns: GridColDef[] = [
    ...
    { field: "year", headerName: "İl", width: 150 },
    {
      field: "status",
      headerName: "Status",
      width: 150,
  ...
  const EditModal: React.FC<Props> = (props) => {
    ...
    const [year, setYear] = useState<string>(props?.data?.year);
    ...
    const handleEdit = async (e: React.FormEvent, id: string) => {
      ...
      try {
        const response = await axios.put(`${endpoint}/kaiyi-history-bottom/${id}`, formData, {
          headers: {
...
export default KaiyiHistoryBottomEdit;
