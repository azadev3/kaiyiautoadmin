...
export type DataTypeKaiyiHistoryBottom = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const KaiyiHistoryBottomShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  } = useQuery<DataTypeKaiyiHistoryBottom[]>({
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeKaiyiHistoryBottom) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-kaiyi-history-bottom/${id}`, {
        status: newStatus,
      });
    ...
    <div className="shower">
      ...
      <div className="table-container">
        <table>
          ...
          <tbody>
            {hasData
              ? fetchData?.map((data: DataTypeKaiyiHistoryBottom) => (
                  <tr key={data?._id}>
                    <td>
                      <div className={`status-btn ${data?.status === "active" ? "actived" : ""}`}>
                        ...
                        <StatusToggle
                          ...
                          api="/status-kaiyi-history-bottom"
                        />
                      </div>
                    </td>
                    <td>{data?._id}</td>
...
export default KaiyiHistoryBottomShow;
