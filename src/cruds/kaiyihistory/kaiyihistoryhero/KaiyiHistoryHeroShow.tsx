...
export type DataTypeKaiyiHistoryHero = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const KaiyiHistoryHeroShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeKaiyiHistoryHero) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-kaiyi-history-hero/${id}`, {
        status: newStatus,
      });
...
export default KaiyiHistoryHeroShow;
