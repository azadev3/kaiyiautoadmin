...
const KaiyiNewsShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  } = useQuery<DataTypeKaiyiNews[]>({
    queryKey: ["fetchDataKeyKaiyiNews"],
    queryFn: async () => {
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeKaiyiNews) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-kaiyi-history-news/${id}`, {
        status: newStatus,
      });
