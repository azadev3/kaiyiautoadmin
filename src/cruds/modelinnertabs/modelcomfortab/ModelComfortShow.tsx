...
export type DataTypeModelComfort = {
  _id: string;
  image: string;
  status?: string;
...
const ModelComfortShow: React.FC = () => {
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeModelComfort) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-modelcomfortab/${id}`, {
        status: newStatus,
      });
              ...
              <tbody>
                {hasData && isTableModal ? (
                  <tr>
                    ...
                    <td title={isTableModal?.image}>{isTableModal?.image}</td>
...
export default ModelComfortShow;
