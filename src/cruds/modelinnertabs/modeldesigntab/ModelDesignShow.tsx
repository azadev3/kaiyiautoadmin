...
export type DataTypeModelDesignTab = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const ModelDesignShow: React.FC = () => {
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeModelDesignTab) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-modeldesigntab/${id}`, {
        status: newStatus,
      });
              ...
              <tbody>
                {hasData && isTableModal ? (
                  <tr>
                    ...
                    <td title={isTableModal?.color}>{isTableModal?.color}</td>
...
export default ModelDesignShow;
