...
export type DataTypeModelSecTab = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const ModelSecTabShow: React.FC = () => {
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeModelSecTab) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-modelsectab/${id}`, {
        status: newStatus,
      });
...
export default ModelSecTabShow;
