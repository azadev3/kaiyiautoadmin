...
const ModelVideoCreate: React.FC = () => {
  ...
  const { refetch } = useQuery<DataTypeModelVideoTab[]>({
    queryKey: ["fetchDataKeyModelVideoTab"],
    queryFn: async () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${endpoint}/modelvideotab`, formData, {
        headers: {
  ...
  return (
    <form acceptCharset="UTF-8" onSubmit={handleSubmit}>
      ...
      <InputImageField labelTitle="Video yüklə" req={true} onChange={handleChange} name="video" accepting={"video/*"} />
      {videoPreview && (
        <video
          src={videoPreview}
          controls={true}
...
export default ModelVideoCreate;
