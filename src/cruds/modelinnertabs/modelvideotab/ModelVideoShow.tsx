...
export type DataTypeModelVideoTab = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const ModelVideoShow: React.FC = () => {
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeModelVideoTab) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-modelvideotab/${id}`, {
        status: newStatus,
      });
