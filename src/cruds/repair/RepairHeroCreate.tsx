...
const RepairHeroCreate: React.FC = () => {
  ...
  const { refetch } = useQuery<DataTypeRepairHero[]>({
    queryKey: ["fetchDataKeyRepairHero"],
    queryFn: async () => {
      const response = await axios.get(`${endpoint}/repair-hero`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${endpoint}/repair-hero`, formData, {
        headers: {
  ...
  return (
    <form acceptCharset="UTF-8" onSubmit={handleSubmit}>
      <InputField
        value={titleAz}
        label="Təmir və Baxım - Hero Açıqlama Başlıq (AZ)"
        placeholder="Məsələn: Example data..."
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
      />
      <InputField
        label="Təmir və Baxım - Hero Açıqlama Başlıq (EN)"
        placeholder="Məsələn: Example data..."
        value={titleEn}
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
      />
      <InputField
        label="Təmir və Baxım - Hero Açıqlama Başlıq (RU)"
        placeholder="Məsələn: Example data..."
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleRu(e.target.value)}
        value={titleRu}
      />
      <InputField
        label="Təmir və Baxım - Hero Açıqlama Açıqlama (AZ) (maksimum: 80 hərf)"
        placeholder="Məsələn: Example data..."
      ...
      />
      <InputField
        label="Təmir və Baxım - Hero Açıqlama Açıqlama (EN) (maksimum: 80 hərf)"
        placeholder="Məsələn: Example data..."
      ...
      />
      <InputField
        label="Təmir və Baxım - Hero Açıqlama Açıqlama (RU) (maksimum: 80 hərf)"
        placeholder="Məsələn: Example data..."
...
export default RepairHeroCreate;
