...
export type DataTypeRepairHero = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const RepairHeroShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  } = useQuery<DataTypeRepairHero[]>({
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeRepairHero) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-repair-hero/${id}`, {
        status: newStatus,
      });
