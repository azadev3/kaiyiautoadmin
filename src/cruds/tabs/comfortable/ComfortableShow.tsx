...
export type DataTypeComfortable = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const ComfortableShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  } = useQuery<DataTypeComfortable[]>({
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeComfortable) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-comfortable/${id}`, {
        status: newStatus,
      });
    ...
    <div className="shower">
      ...
      <div className="table-container">
        <table>
          ...
          <tbody>
            {hasData
              ? fetchData?.map((data: DataTypeComfortable) => (
                  <tr key={data?._id}>
                    <td>
...
export default ComfortableShow;
