...
import { DataTypeDesign } from "./DesignShow";
...
const DesignCreate: React.FC = () => {
  ...
  const { refetch } = useQuery<DataTypeDesign[]>({
    queryKey: ["fetchDataKeyDesignTab"],
    queryFn: async () => {
      const response = await axios.get(`${endpoint}/designtab`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${endpoint}/designtab`, formData, {
        headers: {
  ...
  return (
    <form acceptCharset="UTF-8" onSubmit={handleSubmit}>
      <InputField
        value={titleAz}
        label="Design Başlıq (AZ)"
        placeholder="Məsələn: Example data..."
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
      />
      <InputField
        label="Design Başlıq (EN)"
        placeholder="Məsələn: Example data..."
        value={titleEn}
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
      />
      <InputField
        label="Design Başlıq (RU)"
        placeholder="Məsələn: Example data..."
        ...
        <div className="label-radio">
          <label>
            <input
              ...
              checked={selectedOption === "isVideo"}
            />
        ...
        <div className="label-radio">
          <label>
            <input
              ...
              value="isImage"
              onChange={handleRadioChange}
              checked={selectedOption === "isImage"}
            />
