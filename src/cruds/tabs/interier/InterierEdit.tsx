...
import { DataTypeInterier } from "./InterierShow";
import SelectStatus from "../../../uitils/ui/SelectStatus";

type Props = {
  data: DataTypeInterier;
};

const InterierEdit: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<DataTypeInterier[]>({
    queryKey: ["fetchDataKeyInterierTab"],
    queryFn: async () => {
      const response = await axios.get(`${endpoint}/interiertab`, {
        headers: {
  ...
  const rows = hasData
    ? fetchData.map((data: DataTypeInterier) => ({
        id: data._id,
  ...
  const EditModal: React.FC<Props> = (props) => {
    ...
    const handleEdit = async (e: React.FormEvent, id: string) => {
      ...
      try {
        const response = await axios.put(`${endpoint}/interiertab/${id}`, formData, {
          headers: {
      ...
      <div className="edit-modal">
        ...
        <div className="edit-container">
          <form acceptCharset="UTF-8" onSubmit={(e: React.FormEvent) => handleEdit(e, props?.data?._id)}>
            <InputField
              value={titleAz}
              label="Interier Başlıq (AZ)"
              placeholder="Məsələn: Example data..."
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
            />
            <InputField
              label="Interier Başlıq (EN)"
              placeholder="Məsələn: Example data..."
              value={titleEn}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
            />
            <InputField
              label="Interier Başlıq (RU)"
              placeholder="Məsələn: Example data..."
