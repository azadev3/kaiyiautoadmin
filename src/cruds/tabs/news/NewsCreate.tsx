...
const NewsCreate: React.FC = () => {
  ...
  const { refetch } = useQuery<DataTypeNews[]>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${endpoint}/newstab`, formData, {
  ...
  return (
    <form acceptCharset="UTF-8" onSubmit={handleSubmit}>
      <InputField
        name="title_az"
        value={titleAz}
        label="Xəbərlər Başlıq (AZ)"
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
      />
      <InputField
        name="title_en"
        value={titleEn}
        label="Xəbərlər Başlıq (EN)"
        onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
      />
      <InputField
        name="title_ru"
        value={titleRu}
        label="Xəbərlər Başlıq (RU)"
      ...
      <InputImageField labelTitle="Video yüklə" req={true} accepting="video/*" onChange={handleChangeVideo} name="video" />
