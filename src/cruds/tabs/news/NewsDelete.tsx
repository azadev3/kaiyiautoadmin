...
import { DataTypeNews } from "./NewsShow";

const NewsDelete: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  } = useQuery<DataTypeNews[]>({
    queryKey: ["fetchDataKeyNewsTab"],
    queryFn: async () => {
      const response = await axios.get(`${endpoint}/newstab`, {
        headers: {
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await axios.delete(`${endpoint}/newstab/${id}`);
      if (response.data) {
  ...
  const columns: GridColDef[] = [
    ...
    { field: "slogan_az", headerName: "Sloqan AZ", width: 180 },
    { field: "slogan_en", headerName: "Sloqan EN", width: 180 },
    { field: "slogan_ru", headerName: "Sloqan RU", width: 180 },
  ...
  const rows =
    fetchData && fetchData?.length > 0
      ? fetchData.map((data: DataTypeNews) => ({
          id: data._id,
