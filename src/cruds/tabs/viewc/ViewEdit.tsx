...
import { DataTypeView } from "./ViewShow";
...
const ViewEdit: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<DataTypeView[]>({
    queryKey: ["fetchDataKeyViewTab"],
    queryFn: async () => {
      const response = await axios.get(`${endpoint}/viewtab`, {
        headers: {
  ...
  const rows = hasData
    ? fetchData.map((data: DataTypeView) => ({
        id: data._id,
  ...
  const EditModal: React.FC<Props> = (props) => {
    ...
    const handleEdit = async (e: React.FormEvent, id: string) => {
      ...
      try {
        const response = await axios.put(`${endpoint}/viewtab/${id}`, formData, {
          headers: {
      ...
      <div className="edit-modal">
        ...
        <div className="edit-container">
          <form acceptCharset="UTF-8" onSubmit={(e: React.FormEvent) => handleEdit(e, props?.data?._id)}>
            <InputField
              value={titleAz}
              label="View Başlıq (AZ)"
              placeholder="Məsələn: Example data..."
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
            />
            <InputField
              label="View Başlıq (EN)"
              placeholder="Məsələn: Example data..."
              value={titleEn}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
            />
            <InputField
              label="View Başlıq (RU)"
              placeholder="Məsələn: Example data..."
