...
export type DataTypeTrafficRulesBottom = {
  _id: string;
  description: {
    az: string;
    en: string;
...
const TrafficRulesBottomShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeTrafficRulesBottom) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-traffic-rules-bottom/${id}`, {
        status: newStatus,
      });
                    ...
                    <td>
                      <div className={`status-btn ${data?.status === "active" ? "actived" : ""}`}>
                        ...
                        <StatusToggle
                          ...
                          api="/status-traffic-rules-bottom"
                        />
                      </div>
                    </td>
                    <td>{data?._id}</td>
...
export default TrafficRulesBottomShow;
