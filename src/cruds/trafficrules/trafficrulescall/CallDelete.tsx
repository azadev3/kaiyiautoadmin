...
const CallDelete: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await axios.delete(`${endpoint}/traffic-rules-call/${id}`);
      if (response.data) {
  ...
  const rows =
    fetchData && fetchData?.length > 0
      ? fetchData.map((data: DataTypeTrafficRulesCall) => ({
          ...
          miniTitleAz: data?.miniTitle?.az,
          miniTitleEn: data?.miniTitle?.en,
          miniTitleRu: data?.miniTitle?.ru,
