...
export type DataTypeTrafficRulesCall = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const CallShow: React.FC = () => {
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeTrafficRulesCall) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-traffic-rules-call/${id}`, {
        status: newStatus,
      });
            ...
            {hasData
              ? fetchData?.map((data: DataTypeTrafficRulesCall) => (
                  <tr key={data?._id}>
                    ...
                    <td>{data?.miniTitle?.az}</td>
                    <td>{data?.miniTitle?.en}</td>
                    <td>{data?.miniTitle?.ru}</td>
              ...
              <tbody>
                {hasData && isTableModal ? (
                  <tr>
                    ...
                    <td title={isTableModal?.miniTitle?.az}>{isTableModal?.miniTitle?.az}</td>
                    <td title={isTableModal?.miniTitle?.en}>{isTableModal?.miniTitle?.en}</td>
                    <td title={isTableModal?.miniTitle?.ru}>{isTableModal?.miniTitle?.ru}</td>
                    <td title={isTableModal?.telephone}>{isTableModal?.telephone}</td>
