...
export type DataTypeTrafficRulesHelped = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const TrafficRulesHelpedShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  } = useQuery<DataTypeTrafficRulesHelped[]>({
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeTrafficRulesHelped) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-traffic-rules-helped/${id}`, {
        status: newStatus,
      });
    ...
    <div className="shower">
      ...
      <div className="table-container">
        <table>
          ...
          <tbody>
            {hasData
              ? fetchData?.map((data: DataTypeTrafficRulesHelped) => (
                  <tr key={data?._id}>
                    <td>
                      <div className={`status-btn ${data?.status === "active" ? "actived" : ""}`}>
                        ...
                        <StatusToggle
                          ...
                          api="/status-traffic-rules-helped"
                        />
                      </div>
                    </td>
                    <td>{data?._id}</td>
...
export default TrafficRulesHelpedShow;
