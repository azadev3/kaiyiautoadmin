...
type Props = {
  data: DataTypeTrafficRulesHero;
};

const TrafficRulesHeroEdit: React.FC = () => {
  ...
  const EditModal: React.FC<Props> = (props) => {
    ...
    const handleEdit = async (e: React.FormEvent, id: string) => {
      ...
      try {
        const response = await axios.put(`${endpoint}/traffic-rules-hero/${id}`, formData, {
          headers: {
      ...
      <div className="edit-modal">
        ...
        <div className="edit-container">
          <form acceptCharset="UTF-8" onSubmit={(e: React.FormEvent) => handleEdit(e, props?.data?._id)}>
            <InputField
              value={titleAz}
              label="Yol Qaydaları Hero Başlıq (AZ)"
              placeholder="Məsələn: Example data..."
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleAz(e.target.value)}
            />
            <InputField
              label="Yol Qaydaları Hero Başlıq (EN)"
              placeholder="Məsələn: Example data..."
              value={titleEn}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleEn(e.target.value)}
            />
            <InputField
              label="Yol Qaydaları Hero Başlıq (RU)"
              placeholder="Məsələn: Example data..."
              onChange={(e: ChangeEvent<HTMLInputElement>) => setTitleRu(e.target.value)}
              value={titleRu}
            />
            <InputField
              label="Yol Qaydaları Hero Açıqlama (AZ) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
            ...
            />
            <InputField
              label="Yol Qaydaları Hero Açıqlama (EN) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
            ...
            />
            <InputField
              label="Yol Qaydaları Hero Açıqlama (RU) (maksimum: 80 hərf)"
              placeholder="Məsələn: Example data..."
...
export default TrafficRulesHeroEdit;
