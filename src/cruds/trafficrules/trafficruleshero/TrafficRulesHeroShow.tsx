...
export type DataTypeTrafficRulesHero = {
  _id: string;
  title: {
    az: string;
    en: string;
...
const TrafficRulesHeroShow: React.FC = () => {
  const {
    data: fetchData,
    isLoading,
    refetch,
  ...
  const isTableModal =
    hasData &&
    fetchData?.find((data: DataTypeTrafficRulesHero) => {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      const response = await axios.post(`${endpoint}/status-update-traffic-rules-hero/${id}`, {
        status: newStatus,
      });
...
export default TrafficRulesHeroShow;
